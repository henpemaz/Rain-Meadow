...
{
    public class OnlineAwayFromRoom : OnlinePointer
    {
        ...
        public OnlineAwayFromRoom(OnlinePlayerIndicator jollyHud)
            : base(jollyHud)
        {
        ...
        public void InitiateSprites()
        {
            ...
            {
                shader = indicator.hud.rainWorld.Shaders["Hologram"],
                scale = scale
            });
            sprites.Add(new FSprite("Futile_White")
            {
                shader = indicator.hud.rainWorld.Shaders["FlatLight"],
                alpha = 0f,
                x = -1000f
            });
            for (int i = 0; i < sprites.Count; i++)
            {
                ...
                indicator.fContainer.AddChild(sprites[i]);
            }
        }
        ...
        public override void Update()
        {
...

            {
                if (players[i].realizedCreature != null && (!OnlineManager.players[i].isMe))
                {
                    ...
                    roomPos = players[i].world.RoomToWorldPos(indicator.camPos, indicator.Camera.room.abstractRoom.index);
                }
                ...
                if (indicator.PlayerRoomBeingViewed || indicator.inShortcut || !knownPos || forceHide)
                {
                    hidden = true;
                }
                else if (hidden)
