...
namespace RainMeadow
{
    public class StoryOnlineMenu : SlugcatSelectMenu, IChatSubscriber
    {
        ...
        public SlugcatStats.Name PlayerSelectedSlugcat
        {
            get
            {
                return playerSelectedSlugcats?[0] ?? storyGameMode?.currentCampaign ?? slugcatColorOrder[slugcatPageIndex];
            }
            set
            {
        ...
        {
            if ((playerSelectedSlugcats[player] != slugcat && playerSelectedSlugcats[player] != null) || (playerSelectedSlugcats[player] == null && storyGameMode.currentCampaign != slugcat))
            {
                if (ModManager.JollyCoop)
                {
