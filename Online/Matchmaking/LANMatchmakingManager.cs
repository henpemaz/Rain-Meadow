...
namespace RainMeadow
{
    ...
    public class LANMatchmakingManager : MatchmakingManager
    {
        public class LANLobbyInfo : LobbyInfo
        {
            ...
            public override MatchmakingManager.MatchMakingDomain domain => MatchmakingManager.MatchMakingDomain.LAN;
            protected override object[] JoinCodeArgs => new object[] { endPoint.Address.Address, endPoint.Port };
        }
        ...
        public override MatchMakingDomain domain => MatchMakingDomain.LAN;

        protected override LobbyInfo? LobbyInfoFromJoinArgs(string[] parts, int argsStart)
        {
            if (parts.Length > argsStart + 1 && long.TryParse(parts[argsStart], out var address) && int.TryParse(parts[argsStart + 1], out var port))
                return new LANLobbyInfo(new IPEndPoint(address, port), "", "", 0, false, 4);
            RainMeadow.Error($"found {JoinCodePrefix} but no valid lobby address and port");
            return null;
        }
        ...
        public override void initializeMePlayer()
        {
            if (OnlineManager.netIO is LANNetIO netio)
            {
                ...
                OnlineManager.mePlayer = new OnlinePlayer(new LANPlayerId(new IPEndPoint(
                    UDPPeerManager.getInterfaceAddresses()[0], netio.manager.port)))
                { isMe = true };
