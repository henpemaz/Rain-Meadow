    ...
    public class DataChunk
    {
        ...
        public void Update() // sender logic for processing ackd slices
        {
            ...
            foreach (SliceMessage slice in outgoingSlices)
            {
                ...
                {
                    if (EventMath.IsNewerOrEqual(player.latestTickAck, slice.tick)) // should have arrived
                    {
                        if (EventMath.IsNewer(player.oldestTickToConsider, slice.tick)) // huge ack gap
                        {
                            slice.Resend();
                        }
