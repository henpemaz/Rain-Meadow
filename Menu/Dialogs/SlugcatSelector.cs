...
namespace RainMeadow.UI
{
    public class SlugcatSelector : Dialog
    {
        ...
        public int myRollingCounter,
            ...
            startEndRollingOrderCounter = 40,
            endRollingCounter = 120;
        public float desiredSelectPagePosY,
            congratsLabelAlpha = 0;
        public List<string> losingDescriptions =
            ...
            [
                "Congratulations! You have achieved the ultimate gamble skill!!!",
                "Gahh?? Impossible?!",
            ];

        public static List<string> slugcatSelectorHints =
        [
            "Press and hold something on screen to spin the slots / Hope you brought green",
            "One, two three of a kind / Somewhere on this page you will find",
            "A joke started from the Meadow Discord / Makes all players broke, and none can afford",
        ];
        ...
        public bool IsCloseMatching =>
            slugcatRandomizers
                ...
                .FirstOrDefault()
                ?.Count()
            == slugcatRandomizers.Length - 1;

        public SlugcatSelector(
            ProcessManager manager,
            SlugcatStats.Name[] currentSlugcats,
            SlugcatStats.Name[] currentlySelectableSlugcats,
            Action<SlugcatStats.Name[], SlugcatSelector> recieveSlugcats,
            int perRow = 3,
            float desiredPortraitScale = 1.2f
        )
            : base(manager)
        {
            ...
            continueButton = new(
                this,
                selectPage,
                Translate("LET'S ROLL!"),
                new(
                    manager.rainWorld.options.ScreenSize.x / 2 - 60,
                    PopulateSlugcatRandomiers(
                        currentSlugcats,
                        currentlySelectableSlugcats,
                        perRow,
                        desiredPortraitScale
                    )
                ),
                new(120, 30)
            )
        ...
        {
            if (isClosing || queuedToClose)
                return;
            if (recieveSlugcat)
                RecieveSlugcat?.Invoke(
                    [.. slugcatRandomizers.Select(x => x.slugcatButton.slugcat)],
                    this
                );
        ...
        public void RollingUpdate()
        {
            ...
            if (myRollingCounter >= endRollingCounter)
                rolling = false;
            myRollingCounter++;
        }
        ...
        public float PopulateSlugcatRandomiers(
            SlugcatStats.Name[] currentSlugcats,
        ...
        )
        {
            ...
            int perRow = Mathf.Max(
                    currentSlugcats.Length % 2 == 0 && buttonsPerRow % 2 != 0
                        ? buttonsPerRow - 1
                        : buttonsPerRow,
                    1
                ),
                howManyRows = ((currentSlugcats.Length - 1) / perRow) + 1,
                num = 0,
                howManyVisibleRows = Mathf.Min(howManyRows, 2);
            float yPosMultipler =
                (manager.rainWorld.options.ScreenSize.y) / (howManyVisibleRows + 1);
            while (num < howManyRows)
            {
                int numOfButtonsPassed = num * perRow,
                    xAdder = 0,
                    numOfButtonsInRow = Mathf.Min(
                        perRow,
                        currentSlugcats.Length - numOfButtonsPassed
                    );
                float xMultipler = manager.rainWorld.options.ScreenSize.x / (numOfButtonsInRow + 1);
                float yPos = yPosMultipler * (howManyRows - num);
                for (
                    int i = numOfButtonsPassed;
                    i < currentSlugcats.Length && i < (num + 1) * perRow;
                    i++
                )
                {
                    float xPos =
                        xMultipler
                        * (xAdder + (numOfButtonsInRow % 2 == 0 && perRow % 2 != 0 ? 0.5f : 1));
                    SlugcatRandomizer randomizer = new(
                        this,
                        selectPage,
                        new(xPos, yPos),
                        currentSlugcats[i],
                    ...
                    if (num == howManyVisibleRows - 1)
                        continueButtonYPos =
                            randomizers[i].pos.y + randomizers[i].slugcatButton.pos.y - 80;
                    xAdder++;
                }
                num++;
            }
        ...
        {
            public float desiredResultPosY,
                lastResultPosY,
                resultPosY;
            public int resultsCounter,
                rollingCounter,
                desiredFlipPortraitCounter = 3;
            public bool rolling,
                hasAlreadyRolled,
                flash = true,
                lost;
            ...
            public SlugcatRandomizer(
                ...
                SlugcatStats.Name[] slugcatList,
                float desiredScale = 1.2f
            )
                : base(menu, owner, pos)
            {
                this.slugcatList = slugcatList;
                desiredResultPosY = lastResultPosY = resultPosY = -150;
                float scaleOffset = 100 * desiredScale,
                    offset = scaleOffset * 0.5f;
                slugcatButton = new(
                    menu,
                    this,
                    new(-offset, -offset),
                    new(scaleOffset, scaleOffset),
                    slugcatList.IndexOf(name) == -1 ? slugcatList[0] : name,
                    false
                );
            ...
            public override void Update()
            {
                ...
                if (!rolling)
                {
                    ...
                    {
                        slugcatResult.text = menu.Translate("You got a <SLUGCATNAME>!")
                            .Replace(
                                "<SLUGCATNAME>",
                                menu.Translate(SlugcatStats.getSlugcatName(slugcatButton.slugcat))
                            );
                        desiredResultPosY = slugcatButton.pos.y - 10;
                    }
                    if (resultPosY != desiredResultPosY)
                    {
                        resultPosY = Custom.LerpAndTick(
                            resultPosY,
                            desiredResultPosY,
                            0.15f,
                            0.15f
            ...
            public override void GrafUpdate(float timeStacker)
            {
                ...
                float resultPos = Mathf.Lerp(lastResultPosY, resultPosY, timeStacker),
                    lerp = Mathf.PingPong(resultsCounter * 0.75f, 40) / 40,
                    flashLerp =
                        0.5f - 0.5f * Mathf.Sin((timeStacker + resultsCounter) / 30 * Mathf.PI * 2);
                ...
                slugcatResult.color = Color.Lerp(
                    Color.white,
                    resultsColor == Color.clear ? MenuColorEffect.rgbVeryDarkGrey : resultsColor,
                    flashLerp
                );
                slugcatResult.alpha = hasAlreadyRolled
                    ? Mathf.InverseLerp(desiredResultPosY - 100, desiredResultPosY, resultPos)
                    : 0;
            }
            ...
            public void StopRolling(SlugcatStats.Name? desiredSlugcat) =>
                StopRolling(slugcatList.IndexOf(desiredSlugcat));
        }
    }
}
