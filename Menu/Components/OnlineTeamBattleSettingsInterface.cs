    ...
    public class OnlineTeamBattleSettingsInterface : RectangularMenuObject, SelectOneButton.SelectOneButtonOwner
    {
        ...
        public override void Update()
        {
            ...
            for (int i = 0; i < teamButtons.Length; i++)
            {
                ...
                if (!teamNameBox.held)
                {
                    // update team names on change
                    teamBattleMode.teamNames[actualTeamIndex] = MatchmakingManager.currentInstance.FilterTeamName(teamBattleMode.teamNames[actualTeamIndex]);
                    teamNameBox.value = teamBattleMode.teamNames[actualTeamIndex];
                }
