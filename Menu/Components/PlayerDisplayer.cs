...
namespace RainMeadow.UI.Components
{
    ...
    {
        public PlayerDisplayer(Menu.Menu menu, MenuObject owner, Vector2 pos, List<OnlinePlayer> players, Func<PlayerDisplayer, bool, OnlinePlayer, Vector2, MenuObject> getPlayerButton, int numOfLargeButtonsToView, float xListSize, (float, float) largeButtonHeightSpacing, (float, float) smallButtonHeightSpacing, float scrollSliderSizeYOffset = -40) : base(menu, owner, pos, numOfLargeButtonsToView, xListSize, largeButtonHeightSpacing)
        {
        ...
        public MenuObject[] PopulatePlayerDisplays(ButtonDisplayer buttonDisplayer, bool isLargeDisplay)
        {
            ...
            List<MenuObject> scrollButtons = [];
            for (int i = 0; i < onlinePlayers?.Count; i++)
            {
                MenuObject? scrollButton = getPlayerButton?.Invoke(this, isLargeDisplay, onlinePlayers[i], PositionOfObject(scrollButtons.Count));
        ...
        public Func<PlayerDisplayer, bool, OnlinePlayer, Vector2, MenuObject> getPlayerButton;
